    ...
    try {
        ...
        result.trades.forEach(trade => {
            ...
            row.innerHTML = `
                ...
                <td>${Number(trade.eth_amount || 0).toFixed(4)} ETH</td>
...
async function loadLivePnl() {
    try {
        ...
        const exchanges = result.by_exchange_today || {};
        ...
        setText("todayTradeValue", Number(today.trade_value_usdt || 0).toFixed(2) + " USDT");

        const pnl = Number(today.net_pnl_usdt || 0);
        const pnlEl = document.getElementById("todayNetPnl");
        if (pnlEl) {
            pnlEl.innerText = (pnl >= 0 ? "+" : "") + pnl.toFixed(4) + " USDT";
            pnlEl.classList.toggle("profit-positive", pnl > 0);
            pnlEl.classList.toggle("profit-negative", pnl < 0);
        }

        setText("todayTradeSplit",
            `${Number(today.profitable_trades || 0)} / ${Number(today.loss_trades || 0)}`
        );
        setText("botUptime", result.uptime_text || "--");

        const status = document.getElementById("pnlEngineStatus");
        if (status) {
            status.innerText = result.engine_status || "LIVE ENGINE - STANDBY";
            status.className = "badge " +
                (result.engine_status === "LIVE AUTO-TRADING" ? "badge-green" : "badge-red");
        }

        const updated = document.getElementById("pnlUpdated");
        if (updated) updated.innerText = "Updated " + new Date().toLocaleTimeString();

        const table = document.getElementById("exchangePnlTable");
        if (table) {
            table.innerHTML = "";
            ["Binance", "Bybit", "KuCoin"].forEach(exchange => {
                const d = exchanges[exchange] || {};
                const net = Number(d.net_pnl_usdt || 0);
                const row = document.createElement("tr");
                row.innerHTML = `
                    <td><strong>${exchange}</strong></td>
                    <td>${Number(d.trade_value_usdt || 0).toFixed(2)} USDT</td>
                    <td class="profit-positive">+${Number(d.gross_profit_usdt || 0).toFixed(4)} USDT</td>
                    <td class="${Number(d.gross_loss_usdt || 0) < 0 ? "profit-negative" : ""}">
                        ${Number(d.gross_loss_usdt || 0).toFixed(4)} USDT
                    </td>
                    <td class="${net >= 0 ? "profit-positive" : "profit-negative"}">
                        ${net >= 0 ? "+" : ""}${net.toFixed(4)} USDT
                    </td>
                    <td>${Number(d.trade_count || 0)}</td>
                `;
                table.appendChild(row);
            });
        }
    } catch (error) {
        console.error("Live P&L Monitor Error:", error);
    }
}
...
// Functions are managed by template engine in index.html