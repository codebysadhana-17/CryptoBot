...
class TestArbitrageCalculation(unittest.TestCase):
    def test_depth_vwap_calculation(self):
        import arbitrage
        buy = arbitrage._consume_asks_for_quote([[100.0, 0.02], [101.0, 0.05]], 5.0)
        self.assertIsNotNone(buy)
        base, spent, vwap = buy
        self.assertAlmostEqual(spent, 5.0, places=9)
        self.assertAlmostEqual(base, (0.02 + 3.0/101.0), places=9)
        self.assertAlmostEqual(vwap, 5.0 / (0.02 + 3.0/101.0), places=9)

        sell = arbitrage._consume_bids_for_base([[102.0, 0.02], [101.0, 0.05]], base)
        self.assertIsNotNone(sell)
        sold, received, sell_vwap = sell
        self.assertAlmostEqual(sold, base, places=9)
        self.assertGreater(sell_vwap, vwap)

    def test_no_depth_means_no_route(self):
        import arbitrage
        self.assertIsNone(arbitrage._consume_asks_for_quote([[100.0, 0.001]], 5.0))
